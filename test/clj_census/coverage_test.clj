...
(deftest empty-comparison
  (let [cov (coverage/from-comparison
    ...
    (is (= 0 (:clojure-total (:headline cov))))
...
(deftest zero-canon-vars-in-one-namespace
  (let [cov (coverage/from-comparison
              {:clojure-tag "x" :dialect-tag "y"
               ...
               :namespaces-compared
               {'clojure.empty
                {:in-both #{} :clojure-only #{} :dialect-only #{'foo}
                 :mismatches []}}})
        ns-stat (get-in cov [:per-namespace 'clojure.empty])]
    (is (= 0 (:clojure-total ns-stat)))
